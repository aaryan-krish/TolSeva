        ...
        <div className='max-w-5xl mx-auto flex items-center justify-between'>
          <div>
            <div className='flex items-center gap-2 mb-1'>
              ...
              <span className='text-gold-400 text-sm font-semibold'>Inspector Dashboard</span>
            </div>
        ...
        <div className='flex items-center gap-3 mb-6 flex-wrap'>
          ...
          {[{ key: 'all', label: 'All Assigned' }, { key: 'expired', label: '🔴 Expired' }, { key: 'approaching', label: '🟡 Approaching Expiry' }].map(({ key, label }) => (
            <button key={key} onClick={() => setFilter(key)}
              className={'px-4 py-1.5 rounded-full text-sm font-semibold border transition-colors ' + (filter === key ? 'bg-navy-900 text-white border-navy-900' : 'bg-white text-gray-700 border-gray-300 hover:border-navy-500')}>
              {label}
            </button>
          ))}
