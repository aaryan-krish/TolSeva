    ...
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button
        ...
        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs sm:text-sm font-semibold transition-all border shadow-sm ${
          isHeader
            ? 'bg-navy-800 hover:bg-navy-700 text-white border-gold-500/50 hover:border-gold-400'
            : 'bg-gray-50 hover:bg-gray-100 text-navy-900 border-gray-300'
        }`}
        title="Change language / भाषा बदलें"
        aria-haspopup="true"
        aria-expanded={open}
      >
        <Globe size={16} className={isHeader ? 'text-gold-400' : 'text-navy-700'} />
      ...
      {open && (
        <div className="absolute right-0 mt-1.5 w-60 sm:w-64 rounded-xl shadow-2xl bg-white ring-1 ring-black/10 z-50 overflow-hidden divide-y divide-gray-100 animate-in fade-in duration-150">
          <div className="px-3.5 py-2 bg-navy-900 text-white flex items-center justify-between">
            <span className="text-xs font-bold text-gold-400 uppercase tracking-wider">Select Language / भाषा चुनें</span>
            <span className="text-[10px] bg-navy-800 text-blue-200 px-1.5 py-0.5 rounded">10 Languages</span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center justify-between transition-colors ${
                    active
                      ? 'bg-navy-50 text-navy-900 font-bold border border-navy-200'
                ...
                >
                  ...
                  {active && <Check size={16} className="text-navy-900 flex-shrink-0" />}
                </button>
              );
            })}
          </div>
